...
namespace TestRefreshTableView
{
	...
	public class AlancDragRefreshTableView : UITableView
	{
...
//					Console.WriteLine ("headerView.Frame="+headerView.Frame);
				}
			}
		}
		...
		public AlancDragRefreshTableView (DragRefreshTableSource theSource)
		{
		...
		private void SetSource (DragRefreshTableSource theSource)
		{
			...
			theSource.Scrolling += (locationY) => {
				...
				if (footerViewActionDis > 0) {
					if (footerViewActionDis > theSource.NeedDragDistance) {
				...
				if (locationY < 0) {
					if (locationY < -theSource.NeedDragDistance) {
